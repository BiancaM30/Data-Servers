...
-- Call first function with success
DECLARE
...
BEGIN
  v_cursor := studenti_pkg.get_students_by_sectie_medie(cod_sectia_in => 22, min_media_in => 9.0);
...
-- Call the second function with success
DECLARE
  v_cursor SYS_REFCURSOR;
  v_nume studenti.nume%TYPE;
  v_prenume studenti.prenume%TYPE;
...
BEGIN
  v_cursor := studenti_pkg.get_students_by_grupa_medie(grupa_in => '912', min_media_in => 7.0, max_media_in => 9.0);
  LOOP
...
-- Call the third function with success
DECLARE
  v_cursor SYS_REFCURSOR;
  v_nume studenti.nume%TYPE; 
  v_prenume studenti.prenume%TYPE; 
